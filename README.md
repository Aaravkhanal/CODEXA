...
- **Hardened Opt-in Docker Sandbox**: for BUILD-mode shell execution (`--sandbox` / `CODEXA_SANDBOX=true`). Features default network isolation (`--network none`), CPU & memory limits (2 CPUs, 2GB RAM), and POSIX rootless execution. CLI output surfaces active sandbox policies (`[Sandboxed Execution: docker (network: none | CPUs: 2 | Memory: 2g)]`). See [docs/SANDBOX.md](./docs/SANDBOX.md).
