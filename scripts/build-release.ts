...
const version = process.env.CODEXA_RELEASE_VERSION ?? packageJson.version;
if (!version || !/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(version)) {
  throw new Error(`Invalid CODEXA release version: ${version ?? "missing"}`);
}
...
const outputPath = resolve(outputDirectory, isWindows ? "codexa.exe" : "codexa");
