...
export type CodexaLensActivityTracker = ReturnType<typeof createCodexaLensActivityTracker>;

export function createCodexaLensActivityTracker(cwd: string, startedAt: number) {
  const modifiedFiles = new Set<string>();
  const startedEvents = new Map<string, CodexaLensActivityEvent>();
  const verificationCalls = new Set<string>();

  const start = (call: ToolCall): CodexaLensActivityEvent => {
    ...
    const event: CodexaLensActivityEvent = {
  ...
  const complete = (toolCallId: string, result: string): CodexaLensActivityEvent | null => {
...
function classifyStartedStatus(toolName: string): CodexaLensFileStatus {
