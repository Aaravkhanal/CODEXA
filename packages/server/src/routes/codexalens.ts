...
import { db } from "@codexa/database/client";
import {
  CODEXALENS_TRACE_SCHEMA_VERSION,
...
import { collectPersistedMetrics } from "../codexalens/metrics";
...
const app = new Hono<AuthenticatedEnv>().get("/:sessionId", async (c) => {
  ...
  const nodes: Array<CodexaLensFileNode | CodexaLensExternalNode> = (mcpInspection?.servers ?? []).map((server) => ({
  ...
  const edges: CodexaLensGraphEdge[] = dedupeEdges(timeline.flatMap((event) =>
    event.mcpServer
  ...
  return c.json({
    traceSchemaVersion: CODEXALENS_TRACE_SCHEMA_VERSION,
    cwd: session.cwd ?? "",
    graph: {
      nodes,
      edges,
...
export function collectPersistedActivity(values: unknown[]): CodexaLensActivityEvent[] {
  const events: CodexaLensActivityEvent[] = [];
  ...
  for (const value of values) {
    ...
    for (const partValue of parts) {
      ...
      if (result !== undefined) {
        ...
        const status: CodexaLensFileStatus = part.state === "output-error" || isLegacyFailure(serializedResult)
          ? "failed"
          : startedStatus;
        events.push({
          id: `${id}:completed`,
...
function classifyLegacyStatus(toolName: string): CodexaLensFileStatus {
...
function dedupeEdges(edges: CodexaLensGraphEdge[]) {
