...
const PLAYGROUND_SCENARIOS = {
  ...
  lens: {
    ...
    response: "CodexaLens codebase intelligence visualization workspace is compiled and ready.",
    chips: ["Graph View (F1)", "Workspace (F2)", "Timeline (F3)"],
    statusText: "Initializing CodexaLens codebase intelligence...",
    countText: "Graph compilation successful."
  },
  mcp: {
    prompt: "/mcp",
    files: ["checking .codexa/mcp.json...", "connecting to github-mcp stdio server...", "connecting to weather-api http server..."],
...
export function CODEXAHeroTerminal() {
  ...
  return (
    <MockupStage variant="hero" className="hero-terminal-stage">
      <div className="hero-mockup-composition">
        <div ref={containerRef} className="hero-terminal" role="img" aria-label="CODEXA terminal session">
        <div className="ht-header" aria-hidden="true">
          <div className="ht-dots"><i /><i /><i /></div>
          <span className="ht-path">codexa — terminal</span>
          <div className="ht-header-right">
            <span className="mac-status"><i /> CODEXA</span>
          </div>
        </div>
...
const HERO_TITLE_LINES = ["Code with CODEXA.", "Stay in your terminal."] as const;
  ...
  return (
    <section className="hero section-shell" id="top" style={{ position: "relative" }}>
      ...
      >
        <HeroHoverHeading />
        <p>
          CODEXA brings deep repository understanding, visible tool execution, MCP integrations, and interactive past-activity inspection right into your terminal shell.
        </p>
