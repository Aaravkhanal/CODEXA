...
export type CodexaLensWorkspaceEntry = {
  path: string;
  name: string;
  kind: "directory" | "file";
  depth: number;
...
export type CodexaLensWorkspaceIndex = {
  entries: CodexaLensWorkspaceEntry[];
  files: string[];
  truncated: boolean;
  ignoredCount: number;
};

export type CodexaLensFilePreview = {
  path: string;
  content: string;
  bytes: number;
  truncated: boolean;
...
export type CodexaLensSearchMatch = {
  path: string;
  line: number | null;
  column: number | null;
  preview: string;
...
export type CodexaLensSearchResult = {
  matches: CodexaLensSearchMatch[];
  truncated: boolean;
  scannedFiles: number;
};
...
export function assertSafeWorkspaceRoot(cwd: string) {
  ...
  if (root === resolve(homedir()) || root === parse(root).root) {
    throw new Error("CodexaLens requires a project directory. Start CODEXA inside your repository.");
  }
}

export async function buildWorkspaceIndex(cwd: string): Promise<CodexaLensWorkspaceIndex> {
  assertSafeWorkspaceRoot(cwd);
  const root = await realpath(cwd);
  const ignoreRules = await loadGitIgnoreRules(root);
  const entries: CodexaLensWorkspaceEntry[] = [];
  const files: string[] = [];
  let truncated = false;
  let ignoredCount = 0;
...
export async function readWorkspaceFile(cwd: string, projectPath: string): Promise<CodexaLensFilePreview> {
  const resolved = await resolveWorkspaceFile(cwd, projectPath);
  if (isSensitivePath(projectPath)) throw new Error("CodexaLens does not display sensitive files");
...
export async function searchWorkspace(
  ...
  query: string,
): Promise<CodexaLensSearchResult> {
  const normalizedQuery = query.trim().toLocaleLowerCase();
  if (normalizedQuery.length < 2) {
    return { matches: [], truncated: false, scannedFiles: 0 };
  }

  const matches: CodexaLensSearchMatch[] = [];
  let scannedFiles = 0;
  let scannedBytes = 0;
  let truncated = false;
...
async function resolveWorkspaceFile(cwd: string, projectPath: string) {
  ...
  for (const segment of relativePath.split(/[\\/]/)) {
    ...
    if (info.isSymbolicLink()) throw new Error("CodexaLens does not follow symbolic links");
  }
