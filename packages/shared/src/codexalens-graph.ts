...
export type CodexaLensFileNode = {
  id: string;
  kind: "file";
  path: string;
  label: string;
...
export type CodexaLensExternalNode = {
  id: string;
  kind: "mcp";
  label: string;
...
export type CodexaLensGraphEdge = {
  source: string;
  target: string;
  kind: "import" | "mcp";
};

export type CodexaLensGraph = {
  nodes: CodexaLensFileNode[];
  edges: CodexaLensGraphEdge[];
  truncated: boolean;
};

export async function buildTypeScriptDependencyGraph(cwd: string): Promise<CodexaLensGraph> {
  ...
  const edges: CodexaLensGraphEdge[] = [];
