...
import { CodexaLensWorkspacePanel } from "./codexalens-workspace-panel";

type RemoteCodexaLensSnapshot = InferResponseType<
  (typeof apiClient.codexalens)[":sessionId"]["$get"],
  200
>;
type GraphNode = RemoteCodexaLensSnapshot["graph"]["nodes"][number];
type GraphEdge = RemoteCodexaLensSnapshot["graph"]["edges"][number];
type CodexaLensSnapshot = Omit<RemoteCodexaLensSnapshot, "graph"> & {
  graph: {
    nodes: GraphNode[];
    edges: GraphEdge[];
    truncated: boolean;
...
type CodexaLensView = "graph" | "workspace" | "timeline";

const STATUS_ICON: Record<CodexaLensFileStatus, string> = {
  inspected: "◉",
  modified: "◆",
  failed: "×",
  verified: "✓",
...
function mergeEvents(
  persisted: CodexaLensActivityEvent[],
  live: CodexaLensActivityEvent[],
) {
  const byId = new Map<string, CodexaLensActivityEvent>();
...
export function CodexaLensDialogContent({ sessionId: explicitSessionId }: { sessionId?: string }) {
  const { id: routeSessionId } = useParams();
  const sessionId = explicitSessionId ?? routeSessionId;
  const [snapshot, setSnapshot] = useState<CodexaLensSnapshot | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [selectedNodeId, setSelectedNodeId] = useState<string | null>(null);
  const [replayEnabled, setReplayEnabled] = useState(false);
  const [replayIndex, setReplayIndex] = useState(0);
  const [view, setView] = useState<CodexaLensView>("graph");
  ...
  const loadSnapshot = useCallback(async () => {
    ...
    try {
      ...
      const [response, localGraph] = await Promise.all([
        apiClient.codexalens[":sessionId"].$get({ param: { sessionId } }),
      ...
      } satisfies CodexaLensSnapshot;
      setSnapshot(merged);
      setSelectedNodeId((current) => current ?? merged.graph.nodes[0]?.id ?? null);
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Failed to load CodexaLens");
    }
  }, [sessionId]);
  ...
  const statusByNode = useMemo(() => {
    const statuses = new Map<string, CodexaLensFileStatus>();
  ...
  if (!sessionId) {
    return (
      <box flexDirection="column" flexGrow={1} minHeight={0} gap={1}>
        ...
        <CodexaLensWorkspacePanel activityPaths={EMPTY_PATHS} />
      </box>
    );
  }
  if (error) {
    return (
      <box flexDirection="column" flexGrow={1} minHeight={0} gap={1}>
        ...
        <CodexaLensWorkspacePanel activityPaths={activityPaths} />
      </box>
    );
  }
  if (!snapshot) {
  ...
  return (
    <box flexDirection="column" flexGrow={1} minHeight={0} gap={1}>
      ...
      {view === "workspace" ? (
        <CodexaLensWorkspacePanel activityPaths={activityPaths} initialPath={workspacePath} />
      ) : view === "timeline" ? (
        <box flexDirection="column" flexGrow={1} minHeight={0} gap={1}>
