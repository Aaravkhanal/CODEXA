...
repository="${CODEXA_REPOSITORY:-codexa/codexa}"
install_dir="${CODEXA_INSTALL_DIR:-${HOME}/.local/bin}"
requested_version="${CODEXA_VERSION:-latest}"

command -v curl >/dev/null 2>&1 || {
  echo "CODEXA installer requires curl." >&2
  exit 1
}
...
if [ "$suffix" = "linux-${architecture}-musl" ] && command -v apk >/dev/null 2>&1; then
  ...
  if [ -n "$missing_packages" ]; then
    echo "CODEXA on Alpine Linux requires:${missing_packages}" >&2
...
case "$tag" in
  v*) version="${tag#v}" ;;
  *)
    echo "Could not determine the CODEXA release version." >&2
    exit 1
    ;;
esac

asset="codexa-v${version}-${suffix}.tar.gz"
...
mkdir -p "$install_dir"
cp "${temporary_directory}/codexa" "${install_dir}/codexa"
chmod 755 "${install_dir}/codexa"

echo "CODEXA ${version} installed at ${install_dir}/codexa"
case ":${PATH}:" in
  *":${install_dir}:"*) ;;
  *)
...
if ! installed_version="$("${install_dir}/codexa" --version 2>&1)"; then
