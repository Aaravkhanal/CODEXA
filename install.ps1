param(
  ...
  [string]$InstallDir = "$env:LOCALAPPDATA\CODEXA\bin"
)

$ErrorActionPreference = "Stop"
$Repository = if ($env:CODEXA_REPOSITORY) { $env:CODEXA_REPOSITORY } else { "codexa/codexa" }
$Headers = @{
  "User-Agent" = "CODEXA-Installer"
...
if ([string]::IsNullOrWhiteSpace($VersionValue) -or $VersionValue -eq "latest") {
  try {
    ...
    throw "Unable to resolve the latest CODEXA release from GitHub for $Repository. Check your network connection or download a specific release from https://github.com/$Repository/releases."
  }
...
if ([string]::IsNullOrWhiteSpace($Tag) -or $Tag -notlike "v*") {
  throw "Invalid CODEXA release tag received from GitHub: '$Tag'"
}
$ResolvedVersion = $Tag -replace "^v", ""
$Asset = "codexa-v$ResolvedVersion-$Target.zip"
...
$TemporaryDirectory = Join-Path ([System.IO.Path]::GetTempPath()) ("codexa-" + [guid]::NewGuid())
...
try {
  ...
  Copy-Item (Join-Path $ExtractedPath "codexa.exe") (Join-Path $InstallDir "codexa.exe") -Force
  ...
  Write-Host "CODEXA $ResolvedVersion installed at $InstallDir\codexa.exe"
  & (Join-Path $InstallDir "codexa.exe") --version
} finally {
